...
class TestCrossValidationSplit(unittest.TestCase):
    def test_cross_validation_split(self):
        ...
        n_folds = 5
        cross_validation_split = CrossValidationSplitLoader(extracted.data, n_folds)
        folds = cross_validation_split()

        self.assertEqual(len(folds), n_folds)
        self.assertEqual(len(extracted.data)//n_folds, len(folds[0]))
    ...
    def test_iteration_once(self):
        ...
        self.assertEqual(len(train_set)+len(validation_set)+len(extracted.data)%n_folds, len(extracted.data))
